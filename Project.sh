...
echo ""
echo "<--------------System Info------------>"
echo "Sistema operacional: $(uname -a)" #Verifica a versão do sistema operativo
echo "Nome de utilizador: $(uname -n)" #Verifica o nome de utilizador
arquitetura=$(uname -m) #Identifica a versão do kernel
...
cpu_model=$(grep "model name" /proc/cpuinfo | head -n 1 | awk -F ':' '{print $2}') #Mostra a info do CPU
cpu_cores=$(grep -c "processor" /proc/cpuinfo) #Mostra o númerto de núcleos
